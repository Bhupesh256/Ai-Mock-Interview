  ...
  return (
    <div className='p-10'>
      ...
      {feedbackList&&feedbackList.map((item,index)=>(
         ...
         <CollapsibleTrigger className='p-2 bg-slate-300 rounded-lg flex justify-between my-2 text-left gap-7 w-full'>
         {item.question}<ChevronsUpDown className='h-5 w-5'/>
         </CollapsibleTrigger>
         <CollapsibleContent>
           <div className='flex flex-col gap-2 '>
           <h2 className='text-red-600 p-2 border rounded-lg'><strong>Rating:</strong>{item.rating}</h2>
           <h2 className='p-2 border rounded-lg bg-purple-200 text-sm text-red-900'><strong>Your Answer: </strong>{item.userAns}</h2>
